...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows == 1) {
        ...
        $update_sql = "UPDATE driver_login SET Password = ? WHERE email = ?";
        $update_stmt = $conn->prepare($update_sql);
        $update_stmt->bind_param("ss", $new_password, $email);
        
        if ($update_stmt->execute()) {
            echo "<script>alert('Password reset successful. You can now login.'); window.location.href='driver_login.php';</script>";
                ...
                <div class="card shadow">
                    <div class="card-body p-5">
                        ...
                        <div class="text-center mt-3">
                            <a href="driver_login.php" class="text-decoration-none">Back to Login</a>
                        </div>
                    </div>
                </div>
            </div>
